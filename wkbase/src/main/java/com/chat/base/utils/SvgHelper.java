...
//                }
    ...
    public static Bitmap getBitmap(int res, int width, int height, int color, float scale) {
        try (InputStream stream = WKBaseApplication.getInstance().getContext().getResources().openRawResource(res)) {
    ...
    public static Bitmap getBitmap(File file, int width, int height, int color) {
        ...
        try (FileInputStream stream = new FileInputStream(file)) {
            ...
            SAXParser sp = spf.newSAXParser();
            XMLReader xr = sp.getXMLReader();
            SVGHandler handler = new SVGHandler(width, height, color, false, 1f);
            xr.setContentHandler(handler);
            xr.parse(new InputSource(stream));
            return handler.getBitmap();
        } catch (Exception e) {
    ...
    public static SvgDrawable getDrawable(int resId, int color) {
        try {
            ...
            xr.setContentHandler(handler);
            xr.parse(new InputSource(WKBaseApplication.getInstance().getContext().getResources().openRawResource(resId)));
            return handler.getDrawable();
        } catch (Exception e) {
            return null;
        }
