...
public class ChatReadCountCircleProgressBar extends View {
    ...
    private final Paint outCirclePaint = new Paint();
    private final Paint hookMarkPaint = new Paint();
    private int outCircleColor = Theme.colorAccount;
    private int hookMarkColor = Theme.colorAccount;
    private final int outCircleWith = 5;
    ...
    public ChatReadCountCircleProgressBar(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void drawHookMark(Canvas canvas) {
        // hookMark
        hookMarkPaint.setColor(ContextCompat.getColor(getContext(), hookMarkColor));
        hookMarkPaint.setStrokeWidth(outCircleWith);
        hookMarkPaint.setStyle(Paint.Style.STROKE);
        hookMarkPaint.setAntiAlias(true);
        hookMarkPaint.setStrokeCap(Paint.Cap.ROUND);
        float center = (float) (getHeight() - outCircleWith) / 2;
        canvas.drawLine((float) (center / 1.5), center, center, (float) (center * 1.5), hookMarkPaint);
        canvas.drawLine(center, (float) (center * 1.5), (float) (center * 1.5), (float) (center / 1.3), hookMarkPaint);
    }

    public void setHookMarkColor(int color) {
        hookMarkColor = color;
        hookMarkPaint.setColor(ContextCompat.getColor(getContext(), hookMarkColor));
        invalidate();
    }
    ...
    private void drawCircle(Canvas canvas) {
        ...
        outCirclePaint.setColor(ContextCompat.getColor(getContext(), outCircleColor));
        outCirclePaint.setStyle(Paint.Style.STROKE);
        outCirclePaint.setStrokeWidth(outCircleWith);
        //设置画布背景颜色
        ...
        //画圆
        canvas.drawCircle(cx, cy, (float) (getWidth() - outCircleWith) / 2, outCirclePaint);
    }
